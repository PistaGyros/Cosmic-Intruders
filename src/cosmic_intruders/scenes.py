...
class MainGameScene:
    def __init__(self, root, canvas, main_menu_scene, difficulty: int):
        ...
        self.heart_image = tk.PhotoImage(file="../../media/heart.gif")
    ...
    @staticmethod
    def change_parameters_on_difficulty(difficulty: int):
    ...
    def create_ui(self):
        ...
        self.canvas.create_rectangle(500, 545, 580, 565, width=2)
        self.canvas.create_rectangle(500, 545, 500 + (80 * (self.shoot_timer / 50)), 565, fill="yellow")
        #self.draw_hp()

    def draw_hp(self):
        for i in range(self.hitpoints):
            self.canvas.create_image(100 + i * 30, 540, anchor="nw", image=self.heart_image)
    ...
    def Update(self):
        ...
        if len(self.meteorites) > 0:
            ...
            for meteorite in self.meteorites:
                meteorite.update(self.delta_time, self.player.rectangle, self)
